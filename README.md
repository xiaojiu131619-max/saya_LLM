...
Agent LLM 是一个基于 React/Vite + Tauri 的本地 GGUF 模型启动器。当前项目已经接入 Tauri 后端和 llama.cpp 运行文件，可以在桌面版中扫描本地 `.gguf` 模型、启动 `llama-server.exe`、进行流式聊天，并读取基础硬件状态。
...
```text
app/src-tauri/target/release/agent-llm.exe
```
